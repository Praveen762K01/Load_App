  ...
  static void saveNetworkVideoInGallery(
    ...
    GallerySaver.saveVideo(path).then((success) async {
      if (success == true) {
        ...
        LoadApp.toast("$fileName  " + getTranslated(context, "folder"));
      } else {
        Navigator.of(keyloader.currentContext!, rootNavigator: true).pop();
        LoadApp.toast(getTranslated(context, 'failedtodownload'));
      }
    }).catchError((err) {
      Navigator.of(keyloader.currentContext!, rootNavigator: true).pop();
      LoadApp.toast(err.toString());
    });
  }
