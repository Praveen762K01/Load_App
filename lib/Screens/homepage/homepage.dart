...
import '../recharge_screens/recharge_page.dart';
...
class HomepageState extends State<Homepage>
 ...
 Future<bool> isRecharge()async{
    var recharge=await FirebaseFirestore.instance.collection("razorpay").doc("data").get();
    bool a=recharge.data()!['recharge'];
    return a;
  }

  void fetchRechargeDetaisl() async {
    if (widget.currentUserNo != null) {
      await FirebaseFirestore.instance
          .collection(DbPaths.collectionusers)
          .doc(widget.currentUserNo)
          ...
          .then((value)async {
        try {
          bool a=await isRecharge();
          bool after=value.data()!['rechargeDate']==null?true:DateTime.now().isAfter(value.data()!['rechargeDate'].toDate());
          // bool expiry=value.data()!['rechargeDate']==null?false:DateTime.now().isBefore(value.data()!['rechargeDate'].toDate());
          
          //if current date is less than recharge date
              ...
              Navigator.pushReplacement(
                ...
                MaterialPageRoute(
                    builder: (context) => RechargeScreen(
                          contactno: widget.currentUserNo!,
                          prefs: widget.prefs,
                          doc: widget.doc,
                        )));
  ...
  triggerSharing() {
    ...
    if (_sharedText != null) {
      ...
      if (_sharedFiles!.length > observer.maxNoOfFilesInMultiSharing) {
        LoadApp.toast(getTranslated(context, 'maxnooffiles') +
            ' ' +
  ...
  void listenToNotification() async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      if (message.data['title'] != 'Call Ended' &&
        ...
        LoadApp.toast(getTranslated(this.context, 'newnotifications'));
      } else {
  ...
  Future<bool> onWillPop() {
    ...
    if (now.difference(currentBackPressTime!) > Duration(seconds: 3)) {
      ...
      LoadApp.toast(getTranslated(this.context, 'doubletaptogoback'));
                                                                  ...
                                                                  Navigator.push(
                                                                      ...
                                                                      new MaterialPageRoute(
                                                                          builder: (context) => ProfileSetting(
                                                                                ...
                                                                                type: LoadApp.getAuthenticationType(biometricEnabled, _cachedModel),
                                                                              )));
                                                                },
                                                                currentUserNo:
                                                                    widget
