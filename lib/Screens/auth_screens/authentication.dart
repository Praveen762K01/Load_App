...
class _AuthenticateState extends State<Authenticate> {
  ...
  Widget build(BuildContext context) {
    ...
    return LoadApp.getNTPWrappedWidget(child);
  }
  ...
  _onPasscodeEntered(String enteredPasscode) {
    if (enteredPasscode.length == 4) {
      bool isValid =
          LoadApp.getHashedAnswer(enteredPasscode) == widget.passcode;
      ...
      if (isValid) {
        ...
        if (passcodeTries > 3) {
          LoadApp.toast('Try after ${math.pow(2, passcodeTries - 3)} minutes');
          LoadApp.toast(getTranslated(this.context, 'authfailed'));
          widget.state.pop();
        }
      }
    }
